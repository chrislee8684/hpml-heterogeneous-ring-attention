...
def run_generation_benchmark(model, tokenizer, initial_ids, num_tokens_to_gen, label, device):
    ...
    input_ids = initial_ids.clone().to(device)
    token_times = []
    ...
    prefill_time = (prefill_end - prefill_start) * 1000 # prefill time (one shot, full context)

    if isinstance(out, tuple):
        logits = out[0]
        past_key_value_states = out[1]
    else:
        logits = out.logits
        past_key_value_states = out.past_key_value_states

    # Start decode from last token of the prompt
    last_token_ids = input_ids[:, -1:].clone()
    ...
    for i in range(num_tokens_to_gen):
        ...
        token_times.append(duration_ms) # per-token decode time

        # Unpack output (tuple vs object)
        if isinstance(out, tuple):
            logits = out[0]
            past_key_value_states = out[1]
        else:
        ...
        last_token_ids = next_token_id

        # Decode token for logging (only meaningful on rank 0)
        if rank == 0:
